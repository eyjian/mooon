...
bool CMainHelper::init(int argc, char* argv[])
{
    ...
    _server = mooon::server::create(&_config_impl, &_factory_impl);
    return _server != NULL;
}
...
uint16_t CMainHelper::get_listen_port(int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
        (void)mooon::utils::CStringUtils::string2int(argv[1], port);
    }
